...
public class DriveTrain extends Subsystem {
  // This is to test with the 2018 drive base. The 2019 drive base will use 4
  // Victor SPX controllers for follower motors 1 and 3
  // private final WPI_TalonSRX leftMotor1 = new
  // WPI_TalonSRX(RobotMap.leftMotor1);
  // private final WPI_TalonSRX leftMotor3 = new
  // WPI_TalonSRX(RobotMap.leftMotor3);
  // private final WPI_TalonSRX rightMotor1 = new
  // WPI_TalonSRX(RobotMap.rightMotor1);
  // private final WPI_TalonSRX rightMotor3 = new
  // WPI_TalonSRX(RobotMap.rightMotor3);
  ...
  public void updateDriveLog() {
    Robot.log.writeLog("DriveTrain", "Update Variables",
        "Drive L1 Volts," + leftMotor1.getMotorOutputVoltage() + ",Drive L2 Volts," + leftMotor2.getMotorOutputVoltage()
            ...
            + Robot.pdp.getCurrent(RobotMap.rightMotor3PDP) + ",L Enc Zero," + leftEncoderZero + ",L Enc Ticks,"
            + getLeftEncoderTicks() + ",L Drive Inches," + getLeftEncoderInches() + ",R Enc Zero," + rightEncoderZero
            + ",R Enc Ticks," + getRightEncoderTicks() + ",R Drive Inches," + getRightEncoderInches() + ",High Gear,"
  ...
  public void driveToCrosshair() {
    ...
    /*
     * if (Robot.vision.distanceFromTarget() > 30 && Robot.vision.areaFromCamera !=
     * 0 && lJoystickPercent == 0) { this.robotDrive.tankDrive(lPercentOutput,
     * rPercentOutput); } else if (Robot.vision.distanceFromTarget() > 30 &&
     * Robot.vision.areaFromCamera != 0) { this.robotDrive.tankDrive(lPercentOutput
     * - lJoystickPercent, rPercentOutput - lJoystickPercent); } else {
     * this.robotDrive.tankDrive(0, 0); } Robot.log.writeLog("DriveTrain",
     * "Vision Driving", "Degrees from Target," + xVal + ",Joystick Ouput," +
     * lJoystickPercent + ",Inches from Target," + Robot.vision.distanceFromTarget()
  ...
  public void turnToCrosshair() {
    ...
    Robot.log.writeLog("DriveTrain", "Vision Turning",
        "Degrees from Target," + xVal + ",Target Area," + Robot.vision.areaFromCamera);
  }
  ...
  if(targetAngle == 90 && ((currentAngle <90 && currentAngle > 0) || (currentAngle > -90 && currentAngle < 0))){
    ...
    if(inRightRange == false){
      ...
      double lPercentOutput = fixSpeed + (0.02 * turnAngle);
    double rPercentOutput = fixSpeed - (0.02 * turnAngle);
